// Comprehensive SEO keywords and metadata configurations
export const seoKeywords = {
  // Primary keywords (high volume, competitive)
  primary: [
    "resume builder",
    "free resume builder",
    "ai resume builder", 
    "professional resume",
    "create resume online",
    "ATS optimized resume",
    "resume maker",
    "resume templates",
    "online resume builder",
    "CV maker"
  ],

  // Secondary keywords (medium volume, less competitive)  
  secondary: [
    "resume generator",
    "resume creator",
    "digital resume",
    "modern resume",
    "resume design",
    "resume scanner",
    "resume optimization",
    "job application tool",
    "career builder",
    "professional CV"
  ],

  // Industry-specific keywords
  industries: [
    "tech resume builder",
    "software engineer resume",
    "engineering resume template",
    "IT professional resume",
    "developer resume maker",
    "data scientist resume",
    "healthcare resume builder",
    "nursing resume template",
    "medical professional CV",
    "finance resume creator",
    "accounting resume builder",
    "marketing resume maker",
    "sales professional resume",
    "teacher resume builder",
    "education resume template",
    "manager resume creator",
    "executive resume builder",
    "startup resume template",
    "remote work resume",
    "consulting resume maker"
  ],

  // Feature-based keywords
  features: [
    "ATS friendly resume",
    "applicant tracking system",
    "ATS compliant resume",
    "keyword optimization",
    "resume scoring",
    "AI powered resume",
    "automated resume builder",
    "smart resume maker",
    "resume AI assistant",
    "intelligent resume creator"
  ],

  // Long tail keywords (100 specific phrases)
  longTail: [
    "free AI powered resume builder online",
    "create professional resume in minutes",
    "ATS optimized resume builder free",
    "best free resume maker 2025",
    "AI resume builder that actually works",
    "professional resume templates free download",
    "how to build resume online for free",
    "automated resume builder with AI assistance",
    "free resume creator with ATS optimization",
    "best online resume builder for tech jobs",
    "AI resume maker for software engineers",
    "free professional resume builder no sign up",
    "create ATS friendly resume online free",
    "modern resume builder with AI features",
    "free resume builder for entry level jobs",
    "professional CV maker online free",
    "AI powered resume optimization tool",
    "free resume builder for career change",
    "online resume creator for students",
    "best free ATS resume builder",
    "AI resume builder for remote jobs",
    "professional resume maker for executives",
    "free resume builder with modern templates",
    "automated resume writing with AI",
    "best online CV builder free",
    "AI resume creator for job seekers",
    "free professional resume generator",
    "ATS compliant resume builder online",
    "modern resume maker with AI help",
    "free resume builder for graduates",
    "professional resume creator online free",
    "AI powered CV builder free",
    "best resume builder for tech professionals",
    "free online resume maker with templates",
    "AI resume optimization service free",
    "professional resume builder for healthcare",
    "free ATS resume checker and builder",
    "modern CV creator with AI features",
    "best free resume builder for managers",
    "AI powered resume writing assistant",
    "free professional CV maker online",
    "resume builder that passes ATS systems",
    "AI resume creator for marketing professionals",
    "free online resume builder for teachers",
    "professional resume maker for sales",
    "best AI resume builder for finance",
    "free resume creator with keyword optimization",
    "modern resume builder for startups",
    "AI powered professional resume maker",
    "free ATS friendly resume creator",
    "best online resume builder for nursing",
    "professional CV builder with AI assistance",
    "free resume maker for data scientists",
    "AI resume builder for consulting jobs",
    "modern professional resume creator free",
    "best free CV builder online",
    "AI powered resume template generator",
    "free resume builder for project managers",
    "professional resume maker for engineers",
    "ATS optimized CV builder free",
    "modern resume creator with AI help",
    "best free resume builder for developers",
    "AI resume optimization tool online",
    "free professional resume builder 2025",
    "online resume maker with AI features",
    "best ATS resume builder free",
    "professional CV creator online free",
    "AI powered resume builder for executives",
    "free modern resume maker online",
    "best online resume creator free",
    "AI resume builder for career changers",
    "professional resume generator free",
    "modern CV builder with templates",
    "free AI resume writing service",
    "best resume builder for remote work",
    "professional online CV maker free",
    "AI powered resume creator online",
    "free ATS resume builder tool",
    "modern professional resume maker",
    "best free online resume builder",
    "AI resume optimization platform",
    "professional resume builder online free",
    "free CV creator with AI assistance",
    "modern resume maker for professionals",
    "best AI powered resume builder",
    "free online professional resume creator",
    "ATS compliant CV builder online",
    "AI resume builder for tech industry",
    "professional resume maker free download",
    "modern online resume builder free",
    "best free AI resume creator",
    "professional CV builder online free",
    "free resume builder with AI optimization",
    "modern professional CV maker",
    "best online resume maker free",
    "AI powered professional resume creator",
    "free ATS optimized resume builder",
    "modern resume builder for job seekers",
    "professional online resume maker free",
    "best free CV creator online",
    "AI resume builder for all industries",
    "free professional resume template builder",
    "modern CV creator online free",
    "best AI resume optimization tool",
    "professional resume builder for free",
    "free online modern resume maker",
    "AI resume creator with templates",
    "best professional resume builder online",
    
    // Additional 50+ Long-tail Keywords for Enhanced SEO
    "resume builder with real-time ATS scoring",
    "professional resume maker for career pivots",
    "AI powered resume writer for executives",
    "free resume builder with industry insights",
    "modern CV creator with expert guidance",
    "resume builder for federal government jobs",
    "AI resume optimization for LinkedIn profiles",
    "professional resume maker with cover letters",
    "free ATS resume scanner and builder combo",
    "resume builder for international job applications",
    "AI powered CV maker for academic positions",
    "modern resume creator for creative industries",
    "professional resume builder for military veterans",
    "free resume maker with interview preparation",
    "AI resume writer for nonprofit organizations",
    "resume builder for startup company applications",
    "professional CV creator for consulting firms",
    "modern resume maker for pharmaceutical industry",
    "AI powered resume builder for biotech careers",
    "free resume creator for financial services jobs",
    "professional resume maker for legal careers",
    "AI resume builder for media and entertainment",
    "modern CV creator for renewable energy jobs",
    "resume builder for cybersecurity professionals",
    "AI powered resume maker for blockchain careers",
    "professional resume creator for AI and machine learning",
    "free resume builder for cloud computing jobs",
    "modern CV maker for data science careers",
    "AI resume writer for digital marketing roles",
    "professional resume builder for e-commerce jobs",
    "resume creator for social media management careers",
    "AI powered CV maker for content creation jobs",
    "modern resume builder for UX/UI design careers",
    "professional resume maker for product management",
    "free AI resume creator for sales leadership roles",
    "resume builder for customer success management",
    "AI powered CV creator for operations management",
    "modern resume maker for supply chain careers",
    "professional resume builder for quality assurance",
    "AI resume writer for business development roles",
    "free resume creator for partnership management",
    "modern CV maker for strategic planning careers",
    "AI powered resume builder for change management",
    "professional resume creator for organizational development",
    "resume maker for diversity and inclusion roles",
    "AI resume builder for corporate training careers",
    "modern CV creator for employee engagement roles",
    "professional resume maker for compensation analysis",
    "AI powered resume builder for benefits administration",
    "resume creator for talent acquisition specialists"
  ]
};

// SEO-optimized descriptions for different pages
export const seoDescriptions = {
  home: "Create professional, ATS-optimized resumes for free with Resumy's AI-powered resume builder. Get hired faster with smart resume templates, keyword optimization, AI assistance, and expert guidance. Build modern resumes that pass applicant tracking systems (ATS) and impress hiring managers. Perfect for software engineers, data scientists, marketing professionals, and all career levels. Start building your perfect resume today - no sign-up required!",
  
  analyzeResume: "Analyze and optimize your resume for free with Resumy's AI-powered resume analyzer. Get instant feedback on ATS compatibility, keyword optimization, formatting, and professional presentation. Improve your resume's performance with actionable insights and recommendations from our intelligent resume scanning technology.",
  
  templates: "Choose from professional, ATS-optimized resume templates designed for every industry and career level. Modern, clean designs that pass applicant tracking systems and impress hiring managers. Free download, easy customization, and AI-powered optimization included.",
  
  features: "Discover Resumy's powerful AI resume building features: ATS optimization, keyword analysis, professional templates, real-time editing, industry-specific formatting, and intelligent content suggestions. Everything you need to create a winning resume that gets you hired."
};

// Meta titles optimized for different pages
export const seoTitles = {
  home: "Resumy - Free AI Resume Builder | Create Professional Resumes Online in Minutes",
  analyzeResume: "Free AI Resume Analyzer & ATS Checker | Optimize Your Resume - Resumy",
  templates: "Professional Resume Templates Free | ATS-Optimized Designs - Resumy", 
  features: "AI Resume Builder Features | ATS Optimization & Smart Tools - Resumy"
};
